...
test('useMemo without an input array always runs its function again', () => {
  let callCount = 0;
  function action() {
    const value = useMemo(() => { ++callCount; return 123; });
...
test('useMemo with an empty input array always runs its function again', () => {
  let callCount = 0;
  function action() {
    const value = useMemo(() => { ++callCount; return 123; }, []);
    expect(value).toBe(123);
...
test('useMemo with an identical input array does not run its function again', () => {
  let callCount = 0;
  function action() {
    const value = useMemo(() => { ++callCount; return 123; }, ['sure']);
    expect(value).toBe(123);
  }
  action();
  action();
